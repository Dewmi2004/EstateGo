// src/services/msw/mockAdapter.ts
// A single custom axios adapter used on EVERY platform — Android, iOS, and
// web. There is no real backend, and no `msw` dependency: msw's
// browser/native interceptor internals reference DOM-only globals (like
// `MessageEvent`) that don't exist in Hermes, causing bundling and runtime
// crashes on native. This plain TypeScript adapter has no such dependency —
// it's just functions calling propertyStore/favoriteStore/paymentStore/db,
// wrapped to look like an axios response. Same behavior everywhere,
// guaranteed, with zero platform-specific code paths to maintain.
